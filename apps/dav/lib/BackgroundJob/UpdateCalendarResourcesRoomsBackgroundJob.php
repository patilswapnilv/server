...
class UpdateCalendarResourcesRoomsBackgroundJob extends TimedJob {
		...
		foreach($backends as $backend) {
			...
			foreach($newIds as $newId) {
				try {
					...
					$metadata = [];
					if ($backend instanceof IMetadataProvider) {
			...
			foreach($deletedIds as $deletedId) {
				...
				$this->deleteMetadataFromCache($dbTableMetadata, 'resource_id', $id);
	 ...
	 *
	 * @param IResource|IRoom $backend
	 * @return array
	 */
	private function getAllMetadataOfBackend($backend):array {
		if (!($backend instanceof IMetadataProvider)) {
			return [];
		}

		$keys = $backend->getAllAvailableMetadataKeys();
		$metadata = [];
		foreach($keys as $key) {
			$metadata[$key] = $backend->getMetadataForKey($key);
		}
